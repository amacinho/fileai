...
# Utility class private to FileSystemOperator
class _DirectoryManager:
    ...
    def cleanup_empty_dirs(self, start_path: Path, stop_path: Path) -> None:
        ...
        while current_path != stop_path and self._is_empty_dir(current_path):
            try:
                current_path.rmdir()
                logging.info(
                    f"Deleted empty directory: {current_path.relative_to(stop_path)}"
                )
    ...
    def _is_empty_dir(self, dir_path: Path) -> bool:
        ...
        - It contains no files or directories, OR
        - It only contains other empty directories
        """
        ...
        try:
            for item in dir_path.iterdir():
                if not item.is_dir():
                    return False  
                if not self._is_empty_dir(item):  
                    return False
            return True
        except Exception:
            return False
...
class FileSystemOperator:
    ...
    def __init__(self, input_base_path: Path, output_base_path: Path, remove_input_files=True):
        ...
        self.directory_manager = _DirectoryManager(self.input_base_path)
