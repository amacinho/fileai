...
class TestFileOperator(unittest.TestCase):
    def setUp(self):
        ...
        self.file_operator = FileOperator(self.input_base_path, self.output_base_path)
        # Mock directory manager for cleanup tests
        self.mock_directory_manager = patch('fileai.file_operator.DirectoryManager').start()
        self.file_operator.directory_manager = self.mock_directory_manager.return_value
    ...
    def test_move_file(self, mock_move, mock_mkdir):
        ...
        self.file_operator.directory_manager.cleanup_empty_dirs.assert_called_once_with(
    ...
    def test_remove_file(self, mock_remove):
        ...
        self.assertEqual(self.file_operator.directory_manager.cleanup_empty_dirs.call_count, 1)
    ...
    def test_cleanup_after_operation(self):
        ...
        self.file_operator._cleanup_after_operation(test_path)
        self.file_operator.directory_manager.cleanup_empty_dirs.assert_called_once_with(
        ...
        self.file_operator.directory_manager.reset_mock()
        outside_path = Path("/outside/path.txt")
        self.file_operator._cleanup_after_operation(outside_path)
        self.file_operator.directory_manager.cleanup_empty_dirs.assert_not_called()
    ...
    def test_compute_hash(self):
        ...
        with patch('builtins.open', mock_file):
            hash1 = self.file_operator.compute_hash(self.input_base_path / "file1.txt")
            hash2 = self.file_operator.compute_hash(self.input_base_path / "file2.txt")
        ...
        with patch('builtins.open', side_effect=IOError()), patch('pathlib.Path.exists', return_value=False):
            hash_result = self.file_operator.compute_hash(self.input_base_path / "nonexistent.txt")
    ...
    def test_read_text_content(self):
        ...
        with patch('builtins.open', mock_file):
            content = self.file_operator.read_text_content(self.input_base_path / "test.txt")
            self.assertEqual(content, test_content)
        
        # Test error handling
        with patch('builtins.open', side_effect=OSError()):
            with self.assertRaises(OSError):
                self.file_operator.read_text_content(self.input_base_path / "nonexistent.txt")
